...
def meshing():
    ...
    triangles[0] = ti.Vector([0,1,2])

    edges[0] = ti.Vector([0,1])
    edges[1] = ti.Vector([1,2])
    edges[2] = ti.Vector([0,2])
...
def initialize():
    ...
    for i in range(N):
        x[i] = ti.Vector([center_x + 0.2*ti.cos(i*math.pi*2/3), center_y + 0.2*ti.sin(i*math.pi*2/3)])
...
@ti.kernel
def render(pixels: ti.template(), triangle: Triangle, width: ti.i32, height: ti.i32):
    ...
    for i,j in pixels:
        ...
        color = paint_color*triangle.mask(ti.Vector([coords_x, coords_y]))
...
while window.running:
    ...
    t = Triangle(x[triangles[0][0]], x[triangles[0][1]], x[triangles[0][2]])
    render(pixels, t, width, height)
